...
def load_examples():
    ...
    with tf.name_scope("input_images"):
    	input_images =  tf.image.resize_images(inputs, [a.img_height, a.img_width//3], method=tf.image.ResizeMethod.AREA)
        #input_images = transform(inputs)

    with tf.name_scope("target_images"):
    	target_images =  tf.image.resize_images(targets, [a.img_height, a.img_width//3], method=tf.image.ResizeMethod.AREA)
        #target_images = transform(targets)

    with tf.name_scope("masks"):
        masks =  tf.image.resize_images(masks, [a.img_height, a.img_width//3], method=tf.image.ResizeMethod.AREA)

    print(input_images.shape)
    print(target_images.shape)
    print(masks.shape)
    paths_batch, inputs_batch, targets_batch, masks_batch = tf.train.batch([paths, input_images, target_images, masks], batch_size=a.batch_size)
...
def create_model(inputs, targets, masks):
    ...
    with tf.name_scope("generator_loss"):
        ...
        #new defined loss func
        num_all = a.img_channel * a.img_height * a.img_width / 3.0
        num_pixel = num_all
        num_all = tf.tile([num_all], [a.batch_size])  #[batch_size]

        num_back = tf.reduce_sum(masks, [1,2,3])    #[batch_size]
        num_front = num_all - num_back  #[batch_size]

        num_back = tf.reshape(num_back, [a.batch_size,1,1,1])   #[batch_size,1,1,1]
        num_back = tf.tile(num_back, [1, a.img_height, a.img_width//3, a.img_channel])  #[bacth_size,row,col,cha]
        
        num_front = tf.reshape(num_front, [a.batch_size,1,1,1])   #[batch_size,1,1,1]
        num_front = tf.tile(num_front, [1, a.img_height, a.img_width//3, a.img_channel])  #[bacth_size,row,col,cha]
        
        diff = tf.abs(targets - outputs)
        back_loss = a.back_weight * diff * masks / num_back
        front_loss = a.front_weight * diff * (1-masks) / num_front 
        gen_loss_L1 = tf.reduce_mean((back_loss + front_loss) * num_pixel)
